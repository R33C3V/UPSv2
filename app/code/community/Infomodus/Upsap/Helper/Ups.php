...
class Infomodus_Upsap_Helper_Ups extends Mage_Core_Helper_Abstract
{
    public
    function setParams($lbl, $request)
    {
        $configOptions = new Infomodus_Upsap_Model_Config_Options;
        /*multistore*/
        ...
        $lbl->shiptoStateProvinceCode = Infomodus_Upsap_Helper_Data::escapeXML($request->getDestRegionCode());
        ...
        if (Mage::getStoreConfig('carriers/upsap/company_type', $storeId) == 'upsinfomodus'/*Mage::helper('core')->isModuleOutputEnabled("Infomodus_Upslabel")*/) {
            ...
            $lbl->shipperCountryCode = Infomodus_Upsap_Helper_Data::escapeXML(Mage::getStoreConfig('upslabel/address_' . Mage::getStoreConfig('upslabel/shipping/defaultshipper', $storeId) . '/countrycode', $storeId));
            $lbl->shipperCity = Infomodus_Upsap_Helper_Data::escapeXML(Mage::getStoreConfig('upslabel/address_' . Mage::getStoreConfig('upslabel/shipping/defaultshipper', $storeId) . '/city', $storeId));
            $lbl->shipperStateProvinceCode = Infomodus_Upsap_Helper_Data::escapeXML($configOptions->getProvinceCode(Mage::getStoreConfig('upslabel/address_' . Mage::getStoreConfig('upslabel/shipping/defaultshipper', $storeId) . '/stateprovincecode', $storeId), $lbl->shipperCountryCode));
            $lbl->shipperPostalCode = Infomodus_Upsap_Helper_Data::escapeXML(Mage::getStoreConfig('upslabel/address_' . Mage::getStoreConfig('upslabel/shipping/defaultshipper', $storeId) . '/postalcode', $storeId));
            $lbl->shipperResidential = Infomodus_Upsap_Helper_Data::escapeXML(Mage::getStoreConfig('upslabel/address_' . Mage::getStoreConfig('upslabel/shipping/defaultshipper', $storeId) . '/residential', $storeId)) == "Y"?1:0;

            $lbl->shipfromCountryCode = Infomodus_Upsap_Helper_Data::escapeXML(Mage::getStoreConfig('upslabel/address_' . Mage::getStoreConfig('upslabel/shipping/defaultshipfrom', $storeId) . '/countrycode', $storeId));
            $lbl->shipfromStateProvinceCode = Infomodus_Upsap_Helper_Data::escapeXML($configOptions->getProvinceCode(Mage::getStoreConfig('upslabel/address_' . Mage::getStoreConfig('upslabel/shipping/defaultshipfrom', $storeId) . '/stateprovincecode', $storeId), $lbl->shipfromCountryCode));
            $lbl->shipfromPostalCode = Infomodus_Upsap_Helper_Data::escapeXML(Mage::getStoreConfig('upslabel/address_' . Mage::getStoreConfig('upslabel/shipping/defaultshipfrom', $storeId) . '/postalcode', $storeId));
            ...
            if (Mage::getStoreConfig('upslabel/quantum/adult', $storeId) != 1 || strpos(Mage::getStoreConfig('upslabel/quantum/adult_allow_country', $storeId), $lbl->shiptoCountryCode) !== FALSE) {
                $lbl->adult = Infomodus_Upslabel_Helper_Help::escapeXML(Mage::getStoreConfig('upslabel/quantum/adult', $storeId));
            }
            ...
            $lbl->shipperCity = Infomodus_Upsap_Helper_Data::escapeXML(Mage::getStoreConfig('shipping/origin/city', $storeId));
            ...
            $lbl->shipperStateProvinceCode = Infomodus_Upsap_Helper_Data::escapeXML($region);
            $lbl->shipperPostalCode = Infomodus_Upsap_Helper_Data::escapeXML(Mage::getStoreConfig('shipping/origin/postcode', $storeId));
            $lbl->shipperCountryCode = Infomodus_Upsap_Helper_Data::escapeXML(Mage::getStoreConfig('shipping/origin/country_id', $storeId));

            $lbl->shipfromStateProvinceCode = Infomodus_Upsap_Helper_Data::escapeXML($region);
            $lbl->shipfromPostalCode = Infomodus_Upsap_Helper_Data::escapeXML(Mage::getStoreConfig('shipping/origin/postcode', $storeId));
            $lbl->shipfromCountryCode = Infomodus_Upsap_Helper_Data::escapeXML(Mage::getStoreConfig('shipping/origin/country_id', $storeId));
            ...
            foreach ($shipmentAllItems AS $item) {
                ...
                if (!$item->isDeleted() && !$item->getParentItemId()) {
                    ...
                    for ($ik = 0; $ik < $itemData['qty']; $ik++) {
                        $packer->addItem(new Infomodus_Upslabel_Model_Packer_TestItem($itemData['price'], $myproduct['width'], $myproduct['length'], $myproduct['height'], $itemData['weight'], true));
                    }
                }
            }
